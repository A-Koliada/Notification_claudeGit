  ...
  async getUserIp() {
    try {
      ...
      if (response.ok) {
        ...
        if (data.ip) {
          LOG("✅ Got user IP:", data.ip);
          return data.ip;
        }
      }
    } catch (e) {
      LOG("⚠️ Could not get IP from ipify:", e.message);
    }
    
    // Fallback - спробуємо ipapi.co
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.ip) {
          LOG("✅ Got user IP from ipapi:", data.ip);
          return data.ip;
        }
      }
    } catch (e) {
      LOG("⚠️ Could not get IP from ipapi:", e.message);
    }
    
    LOG("⚠️ Could not determine user IP");
    return "";
  }
  ...
  async registerOrUpdateUser(additionalFields = {}) {
    ...
    try {
      ...
      const userData = {
        ...
        DnLastIp: userIp, // ✅ Тепер заповнюється!
      ...
      LOG("📝 User data to save:", {
        ...userData,
        DnLastIp: userIp ? userIp : "(empty)"
      });
